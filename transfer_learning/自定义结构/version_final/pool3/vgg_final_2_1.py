...
images2=np.load('photos/beta_data_56/codes_beta_pool3_4w.npy')
images3=np.load('photos/beta_data_56/codes_beta_pool3_6w.npy')
...
x_image = Batch_Norm_(x_image,0,out_size=[14,14,256])
...
h_pool4_flat = tf.reshape(h_pool2, [-1, 4*4*256])
h_pool3 = Batch_Norm_(h_pool4_flat,3,out_size=[4*4*256])


h_fc1 =fc_layer(h_pool3,4*4*256,1024,6,activation_function=tf.nn.relu)[0]
...
saver.save(sess, "beta_try_2/pool3/pool3.ckpt")
...
np.save('beta_try_2/pool3/compute_accuracy_x',compute_accuracy(images_test,labels_test)[1])
np.save('beta_try_2/pool3/compute_accuracy_y',compute_accuracy(images_test,labels_test)[2])


np.save('beta_try_2/pool3/train_loss_',train_loss_)
np.save('beta_try_2/pool3/val_loss_',val_loss_)
...
plt.savefig("beta_try_2/pool3/val_train_pool3.jpg")
...
plt.legend(loc="lower right")
plt.show()
plt.savefig("beta_try_2/pool3/roc_pool3.jpg")
'''
...
print(test_proba)
test_proba.to_csv('beta_try_2/pool3/pool3_submission.csv', index=False, float_format='%.6f')
