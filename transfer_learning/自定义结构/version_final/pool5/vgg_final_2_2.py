...
h_pool1 = Batch_Norm_(h_pool1,1,out_size=[4,4,512])
'''
...
h_pool4_flat = tf.reshape(h_pool1, [-1, 4*514*2])
...
np.save('beta_try_3/pool5/compute_accuracy_x',compute_accuracy(images_test,labels_test)[1])
np.save('beta_try_3/pool5/compute_accuracy_y',compute_accuracy(images_test,labels_test)[2])

np.save('beta_try_3/pool5/compute_accuracy_acc',compute_accuracy(images_test,labels_test)[0])
...
plt.subplot(2,1,1)
plt.plot(range(0,210),train_loss_,label = "train loss")
plt.plot(range(0,210),val_loss_,label = " Validation loss")
...
print(test_proba)
test_proba.to_csv('beta_try_3/pool5/pool5_submission.csv', index=False, float_format='%.6f')
'''
saver.save(sess, "beta_try_2/pool5/pool5.ckpt")
...
plt.savefig("beta_try_2/pool5/val_train_pool5.jpg")
...
plt.legend(loc="lower right")
plt.show()
plt.savefig("beta_try_2/pool5/roc_pool5.jpg")
